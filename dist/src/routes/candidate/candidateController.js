...
export const createForm = async (req, res) => {
    try {
        const { fullname, lastname, dateOfBirth, gender, passportNumber, email, phoneNumber, department, position, cv, // PDF file (base64)
        ...
        const cvUploadResponse = await cloudinary.uploader.upload(`data:application/pdf;base64,${cvBase64}`, { resource_type: "raw", folder: "candidates/cv" } // Store as raw file in Cloudinary
        );
        // Upload Certificate PDF to Cloudinary
        const certificateUploadResponse = await cloudinary.uploader.upload(`data:application/pdf;base64,${certificateBase64}`, { resource_type: "raw", folder: "candidates/certificate" } // Store as raw file in Cloudinary
        );
